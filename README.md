...
Follow the steps outlined in https://www.oscar-system.org/install to install Oscar (and Julia).  When installing Oscar, use the dev branch:
```
]add Oscar#master
```
Then, install Transducers.jl (https://juliafolds.github.io/Transducers.jl/dev/):
```
]add Transducers
```
...
## Notes
Currently, I can't seem to get multithreading working.  For now, make sure not to boot up Julia with multiple threads, lest segfault.
